...
20. Apps hardcode 80×25 while video can be 1080p (edit/less/fm)
    Framebuffer modes exist; UIs still assume classic text geometry.
    Fix: Query console size from the API (add get_term_size) and layout from that.
    Until then, default SYSTEM.INI to text mode for interactive apps.
