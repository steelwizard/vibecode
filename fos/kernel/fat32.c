#include "fat32.h"
#include "block.h"
#include "string.h"

static uint8_t sector_buf[512];

static int read_sector(int block_index, uint32_t lba) {
    const block_dev_t *dev = block_device(block_index);
    if (!dev) {
        return -1;
    }
    return block_read(dev, lba, sector_buf);
}

static int read_sector_vol(const fat32_vol_t *vol, uint32_t lba) {
    return read_sector(vol->block_index, lba);
}

static int write_sector_vol(const fat32_vol_t *vol, uint32_t lba) {
    const block_dev_t *dev = block_device(vol->block_index);
    if (!dev) {
        return -1;
    }
    return block_write(dev, lba, sector_buf);
}

static void wr16(uint8_t *p, uint16_t v) {
    p[0] = (uint8_t)v;
    p[1] = (uint8_t)(v >> 8);
}

static void wr32(uint8_t *p, uint32_t v) {
    p[0] = (uint8_t)v;
    p[1] = (uint8_t)(v >> 8);
    p[2] = (uint8_t)(v >> 16);
    p[3] = (uint8_t)(v >> 24);
}

static uint32_t cluster_lba(const fat32_vol_t *vol, uint32_t cluster) {
    return vol->data_begin_lba + (cluster - 2) * vol->sectors_per_cluster;
}

static uint32_t fat32_next_cluster(const fat32_vol_t *vol, uint32_t cluster) {
    uint32_t fat_offset = cluster * 4;
    uint32_t fat_sector = vol->fat_begin_lba + fat_offset / vol->bytes_per_sector;
    uint32_t ent_off = fat_offset % vol->bytes_per_sector;
    if (read_sector_vol(vol, fat_sector) != 0) {
        return 0x0FFFFFFF;
    }
    uint32_t next =
        *(uint32_t *)(sector_buf + ent_off) & 0x0FFFFFFF;
    return next;
}

static uint16_t rd16(const uint8_t *p) {
    return (uint16_t)p[0] | ((uint16_t)p[1] << 8);
}

static uint32_t rd32(const uint8_t *p) {
    return (uint32_t)p[0] | ((uint32_t)p[1] << 8) |
           ((uint32_t)p[2] << 16) | ((uint32_t)p[3] << 24);
}

int fat32_mount(fat32_vol_t *vol, int block_index, uint32_t part_lba) {
    vol->block_index = block_index;
    if (read_sector(block_index, part_lba) != 0) {
        return -1;
    }

    if (memcmp(sector_buf + 82, "FAT32   ", 8) != 0 &&
        memcmp(sector_buf + 82, "FAT     ", 8) != 0) {
        return -1;
    }

    vol->lba_start = part_lba;
    vol->bytes_per_sector = rd16(sector_buf + 11);
    vol->sectors_per_cluster = sector_buf[13];
    vol->reserved_sectors = rd16(sector_buf + 14);
    vol->num_fats = sector_buf[16];
    vol->fat_size_sectors = rd32(sector_buf + 36);
    vol->root_cluster = rd32(sector_buf + 44);

    if (vol->bytes_per_sector != 512 || vol->sectors_per_cluster == 0 ||
        vol->num_fats == 0) {
        return -1;
    }

    vol->fat_begin_lba = part_lba + vol->reserved_sectors;
    vol->data_begin_lba = vol->fat_begin_lba + vol->num_fats * vol->fat_size_sectors;
    vol->total_clusters = vol->fat_size_sectors * (vol->bytes_per_sector / 4);
    {
        uint16_t fsinfo = rd16(sector_buf + 48);
        if (fsinfo != 0 && fsinfo != 0xFFFF && fsinfo < vol->reserved_sectors) {
            vol->fsinfo_lba = part_lba + fsinfo;
        } else {
            vol->fsinfo_lba = 0;
        }
    }
    return 0;
}

void fat32_unmount(fat32_vol_t *vol) {
    (void)vol;
}

/* Read FSInfo sector for free cluster count (returns 0 if unknown). */
uint64_t fat32_free_bytes(const fat32_vol_t *vol) {
    if (vol->fsinfo_lba == 0) {
        return 0;
    }
    if (read_sector_vol(vol, vol->fsinfo_lba) != 0) {
        return 0;
    }
    if (rd32(sector_buf + 0) != 0x41615252) {
        return 0;
    }
    uint32_t free_count = rd32(sector_buf + 0x1E8);
    if (free_count == 0xFFFFFFFF || free_count == 0) {
        return 0;
    }
    return (uint64_t)free_count * vol->sectors_per_cluster * vol->bytes_per_sector;
}

static void fat32_fsinfo_adjust(fat32_vol_t *vol, int delta) {
    uint32_t free_count;
    uint32_t next_free;

    if (!vol || vol->fsinfo_lba == 0 || delta == 0) {
        return;
    }
    if (read_sector_vol(vol, vol->fsinfo_lba) != 0) {
        return;
    }
    if (rd32(sector_buf + 0) != 0x41615252 || rd32(sector_buf + 0x1E4) != 0x61417272) {
        return;
    }
    free_count = rd32(sector_buf + 0x1E8);
    next_free = rd32(sector_buf + 0x1EC);
    if (free_count != 0xFFFFFFFF) {
        if (delta < 0 && free_count < (uint32_t)(-delta)) {
            free_count = 0;
        } else {
            free_count = (uint32_t)((int32_t)free_count + delta);
        }
        wr32(sector_buf + 0x1E8, free_count);
    }
    if (delta < 0 && next_free != 0xFFFFFFFF) {
        /* hint is stale; leave it — Windows will rescan */
        (void)next_free;
    }
    wr32(sector_buf + 0x1FC, 0xAA550000);
    write_sector_vol(vol, vol->fsinfo_lba);
}

static void format_83_name(const uint8_t *raw, char *out, size_t sz) {
    char base[9];
    char ext[4];
    int i;
    for (i = 0; i < 8; i++) {
        base[i] = raw[i];
    }
    base[8] = 0;
    for (i = 7; i >= 0 && base[i] == ' '; i--) {
        base[i] = 0;
    }
    for (i = 0; i < 3; i++) {
        ext[i] = raw[8 + i];
    }
    ext[3] = 0;
    for (i = 2; i >= 0 && ext[i] == ' '; i--) {
        ext[i] = 0;
    }

    if (ext[0]) {
        size_t n = 0;
        for (i = 0; base[i] && n + 1 < (int)sz; i++) {
            out[n++] = base[i];
        }
        if (n + 1 < sz) {
            out[n++] = '.';
        }
        for (i = 0; ext[i] && n + 1 < (int)sz; i++) {
            out[n++] = ext[i];
        }
        out[n] = 0;
    } else {
        strncpy(out, base, sz);
        out[sz - 1] = 0;
    }
}

static int walk_dir_cluster(fat32_vol_t *vol, uint32_t cluster, fat32_dir_cb cb, void *ctx) {
    while (cluster >= 2 && cluster < 0x0FFFFFF8) {
        uint32_t lba = cluster_lba(vol, cluster);
        for (uint32_t s = 0; s < vol->sectors_per_cluster; s++) {
            if (read_sector_vol(vol, lba + s) != 0) {
                return -1;
            }
            for (int i = 0; i < 512; i += 32) {
                uint8_t *e = sector_buf + i;
                if (e[0] == 0) {
                    return 0;
                }
                if (e[0] == 0xE5 || e[11] == 0x0F) {
                    continue;
                }
                char name[13];
                format_83_name(e, name, sizeof(name));
                uint32_t size = rd32(e + 28);
                uint32_t cl = ((uint32_t)rd16(e + 20) << 16) | rd16(e + 26);
                uint8_t attr = e[11];
                (void)cl;
                if (cb(name, attr, size, ctx) != 0) {
                    return 0;
                }
            }
        }
        cluster = fat32_next_cluster(vol, cluster);
    }
    return 0;
}

int fat32_list_dir(fat32_vol_t *vol, uint32_t cluster, fat32_dir_cb cb, void *ctx) {
    return walk_dir_cluster(vol, cluster, cb, ctx);
}

static int match_component(const char *name, const char *want) {
    return strcasecmp(name, want) == 0;
}

int fat32_find_path(fat32_vol_t *vol, uint32_t start_cluster, const char *path,
                    uint32_t *out_cluster, int *is_dir) {
    uint32_t cluster = start_cluster;
    char comp[64];
    const char *p = path;

    while (*p == '\\') {
        p++;
    }

    if (*p == 0) {
        *out_cluster = cluster;
        *is_dir = 1;
        return 0;
    }

    for (;;) {
        while (*p == '\\') {
            p++;
        }
        if (*p == 0) {
            *out_cluster = cluster;
            *is_dir = 1;
            return 0;
        }

        size_t n = 0;
        while (*p && *p != '\\' && n + 1 < sizeof(comp)) {
            comp[n++] = *p++;
        }
        comp[n] = 0;

        int found = 0;
        uint32_t next_cluster = 0;
        uint8_t attr = 0;
        uint32_t scan = cluster;

        while (scan >= 2 && scan < 0x0FFFFFF8 && !found) {
            uint32_t lba = cluster_lba(vol, scan);
            for (uint32_t s = 0; s < vol->sectors_per_cluster && !found; s++) {
                if (read_sector_vol(vol, lba + s) != 0) {
                    return -1;
                }
                for (int i = 0; i < 512; i += 32) {
                    uint8_t *e = sector_buf + i;
                    if (e[0] == 0) {
                        scan = 0x0FFFFFF8;
                        break;
                    }
                    if (e[0] == 0xE5 || e[11] == 0x0F) {
                        continue;
                    }
                    char name[13];
                    format_83_name(e, name, sizeof(name));
                    if (match_component(name, comp)) {
                        next_cluster = ((uint32_t)rd16(e + 20) << 16) | rd16(e + 26);
                        attr = e[11];
                        found = 1;
                        break;
                    }
                }
            }
            if (!found && scan < 0x0FFFFFF8) {
                scan = fat32_next_cluster(vol, scan);
            }
        }

        if (!found) {
            return -1;
        }

        cluster = next_cluster;
        if (*p == 0) {
            *out_cluster = cluster;
            *is_dir = (attr & FAT_ATTR_DIR) != 0;
            return 0;
        }
        if (!(attr & FAT_ATTR_DIR)) {
            return -1;
        }
    }
}

static int lookup_dirent(fat32_vol_t *vol, uint32_t dir_cluster, const char *name,
                         uint32_t *out_cluster, uint8_t *out_attr, uint32_t *out_size,
                         uint32_t *out_lba, int *out_off) {
    uint32_t scan = dir_cluster;
    while (scan >= 2 && scan < 0x0FFFFFF8) {
        uint32_t lba = cluster_lba(vol, scan);
        for (uint32_t s = 0; s < vol->sectors_per_cluster; s++) {
            if (read_sector_vol(vol, lba + s) != 0) {
                return -1;
            }
            for (int i = 0; i < 512; i += 32) {
                uint8_t *e = sector_buf + i;
                if (e[0] == 0) {
                    return -1;
                }
                if (e[0] == 0xE5 || e[11] == 0x0F) {
                    continue;
                }
                char nm[13];
                format_83_name(e, nm, sizeof(nm));
                if (match_component(nm, name)) {
                    if (out_cluster) {
                        *out_cluster = ((uint32_t)rd16(e + 20) << 16) | rd16(e + 26);
                    }
                    if (out_attr) {
                        *out_attr = e[11];
                    }
                    if (out_size) {
                        *out_size = rd32(e + 28);
                    }
                    if (out_lba) {
                        *out_lba = lba + s;
                    }
                    if (out_off) {
                        *out_off = i;
                    }
                    return 0;
                }
            }
        }
        scan = fat32_next_cluster(vol, scan);
    }
    return -1;
}

int fat32_lookup(fat32_vol_t *vol, uint32_t dir_cluster, const char *name,
                 uint32_t *out_cluster, uint8_t *out_attr, uint32_t *out_size) {
    return lookup_dirent(vol, dir_cluster, name, out_cluster, out_attr, out_size, 0, 0);
}

static void fat32_set_cluster(fat32_vol_t *vol, uint32_t cluster, uint32_t value) {
    uint32_t fat_offset = cluster * 4;
    uint32_t fat_sector = vol->fat_begin_lba + fat_offset / vol->bytes_per_sector;
    uint32_t ent_off = fat_offset % vol->bytes_per_sector;
    if (read_sector_vol(vol, fat_sector) != 0) {
        return;
    }
    uint32_t old = rd32(sector_buf + ent_off);
    wr32(sector_buf + ent_off, (old & 0xF0000000) | (value & 0x0FFFFFFF));
    write_sector_vol(vol, fat_sector);
    if (vol->num_fats > 1) {
        write_sector_vol(vol, fat_sector + vol->fat_size_sectors);
    }
}

static uint32_t fat32_alloc_cluster(fat32_vol_t *vol) {
    for (uint32_t c = 2; c < vol->total_clusters + 2; c++) {
        if (fat32_next_cluster(vol, c) == 0) {
            fat32_set_cluster(vol, c, 0x0FFFFFFF);
            fat32_fsinfo_adjust(vol, -1);
            return c;
        }
    }
    return 0;
}

static void fat32_free_chain(fat32_vol_t *vol, uint32_t cluster) {
    int freed = 0;
    while (cluster >= 2 && cluster < 0x0FFFFFF8) {
        uint32_t next = fat32_next_cluster(vol, cluster);
        fat32_set_cluster(vol, cluster, 0);
        cluster = next;
        freed++;
    }
    if (freed) {
        fat32_fsinfo_adjust(vol, freed);
    }
}

static void name_to_83(const char *name, uint8_t out[11]) {
    char base[9];
    char ext[4];
    int i;
    for (i = 0; i < 8; i++) {
        base[i] = ' ';
    }
    base[8] = 0;
    for (i = 0; i < 3; i++) {
        ext[i] = ' ';
    }
    ext[3] = 0;

    const char *dot = 0;
    for (const char *p = name; *p; p++) {
        if (*p == '.') {
            dot = p;
        }
    }

    int bi = 0;
    for (const char *p = name; *p && p != dot && bi < 8; p++) {
        char c = *p;
        if (c >= 'a' && c <= 'z') {
            c = (char)(c - 32);
        }
        if (c == '\\' || c == '/' || c == ':') {
            continue;
        }
        base[bi++] = c;
    }

    int ei = 0;
    if (dot) {
        for (const char *p = dot + 1; *p && ei < 3; p++) {
            char c = *p;
            if (c >= 'a' && c <= 'z') {
                c = (char)(c - 32);
            }
            ext[ei++] = c;
        }
    }

    for (i = 0; i < 8; i++) {
        out[i] = (uint8_t)base[i];
    }
    for (i = 0; i < 3; i++) {
        out[8 + i] = (uint8_t)ext[i];
    }
}

static int fat32_find_dir_slot(fat32_vol_t *vol, uint32_t dir_cluster,
                               uint32_t *out_lba, int *out_off) {
    uint32_t last = dir_cluster;

    while (dir_cluster >= 2 && dir_cluster < 0x0FFFFFF8) {
        uint32_t lba = cluster_lba(vol, dir_cluster);
        last = dir_cluster;
        for (uint32_t s = 0; s < vol->sectors_per_cluster; s++) {
            if (read_sector_vol(vol, lba + s) != 0) {
                return -1;
            }
            for (int i = 0; i < 512; i += 32) {
                uint8_t *e = sector_buf + i;
                if (e[0] == 0x00 || e[0] == 0xE5) {
                    *out_lba = lba + s;
                    *out_off = i;
                    return 0;
                }
            }
        }
        dir_cluster = fat32_next_cluster(vol, dir_cluster);
    }

    /* Directory full — grow by one cluster and use the first slot. */
    {
        uint32_t extra = fat32_alloc_cluster(vol);
        uint32_t lba;
        uint32_t s;

        if (extra < 2 || last < 2) {
            return -1;
        }
        fat32_set_cluster(vol, last, extra);
        lba = cluster_lba(vol, extra);
        memset(sector_buf, 0, 512);
        for (s = 0; s < vol->sectors_per_cluster; s++) {
            if (write_sector_vol(vol, lba + s) != 0) {
                return -1;
            }
        }
        *out_lba = lba;
        *out_off = 0;
        return 0;
    }
}

/* Flags match VFS_O_*. */
#define F_READ   0x01
#define F_WRITE  0x02
#define F_CREATE 0x04
#define F_TRUNC  0x08
#define F_APPEND 0x10

static uint32_t fat32_cluster_size(const fat32_vol_t *vol) {
    return vol->sectors_per_cluster * vol->bytes_per_sector;
}

static int fat32_sync_dirent(fat32_vol_t *vol, fat32_file_t *f) {
    uint8_t *e;

    if (read_sector_vol(vol, f->dir_lba) != 0) {
        return -1;
    }
    e = sector_buf + f->dir_off;
    wr16(e + 26, (uint16_t)(f->start_cluster & 0xFFFF));
    wr16(e + 20, (uint16_t)(f->start_cluster >> 16));
    wr32(e + 28, f->size);
    if (write_sector_vol(vol, f->dir_lba) != 0) {
        return -1;
    }
    f->dirty = 0;
    return 0;
}

static int fat32_zero_cluster(fat32_vol_t *vol, uint32_t cluster) {
    uint32_t lba = cluster_lba(vol, cluster);
    uint32_t s;

    memset(sector_buf, 0, vol->bytes_per_sector);
    for (s = 0; s < vol->sectors_per_cluster; s++) {
        if (write_sector_vol(vol, lba + s) != 0) {
            return -1;
        }
    }
    return 0;
}

static int fat32_rw_cluster(fat32_vol_t *vol, uint32_t cluster, uint32_t offset,
                            uint8_t *data, const uint8_t *src, uint32_t len) {
    uint32_t bps = vol->bytes_per_sector;
    uint32_t lba = cluster_lba(vol, cluster);
    uint32_t pos = 0;

    while (pos < len) {
        uint32_t abs = offset + pos;
        uint32_t s = abs / bps;
        uint32_t b = abs % bps;
        uint32_t n = bps - b;
        int writing = src != 0;

        if (n > len - pos) {
            n = len - pos;
        }
        if (writing && n == bps && b == 0) {
            memcpy(sector_buf, src + pos, bps);
            if (write_sector_vol(vol, lba + s) != 0) {
                return -1;
            }
        } else {
            if (read_sector_vol(vol, lba + s) != 0) {
                return -1;
            }
            if (writing) {
                memcpy(sector_buf + b, src + pos, n);
                if (write_sector_vol(vol, lba + s) != 0) {
                    return -1;
                }
            } else {
                memcpy(data + pos, sector_buf + b, n);
            }
        }
        pos += n;
    }
    return 0;
}

static int fat32_cluster_at(fat32_vol_t *vol, fat32_file_t *f, uint32_t idx) {
    uint32_t cl;
    uint32_t i;

    if (f->start_cluster < 2) {
        return -1;
    }
    if (f->clus >= 2 && f->clus_idx == idx) {
        return 0;
    }
    if (f->clus >= 2 && idx > f->clus_idx) {
        cl = f->clus;
        i = f->clus_idx;
    } else {
        cl = f->start_cluster;
        i = 0;
    }
    while (i < idx) {
        uint32_t next = fat32_next_cluster(vol, cl);
        if (next < 2 || next >= 0x0FFFFFF8) {
            return -1;
        }
        cl = next;
        i++;
    }
    f->clus = cl;
    f->clus_idx = idx;
    return 0;
}

static int fat32_ensure_cluster(fat32_vol_t *vol, fat32_file_t *f, uint32_t idx) {
    uint32_t cl;
    uint32_t i;

    if (f->start_cluster < 2) {
        cl = fat32_alloc_cluster(vol);
        if (cl < 2) {
            return -1;
        }
        fat32_set_cluster(vol, cl, 0x0FFFFFFF);
        if (fat32_zero_cluster(vol, cl) != 0) {
            return -1;
        }
        f->start_cluster = cl;
        f->clus = cl;
        f->clus_idx = 0;
        f->dirty = 1;
        if (fat32_sync_dirent(vol, f) != 0) {
            return -1;
        }
        if (idx == 0) {
            return 0;
        }
    }

    if (f->clus >= 2 && f->clus_idx <= idx) {
        cl = f->clus;
        i = f->clus_idx;
    } else {
        cl = f->start_cluster;
        i = 0;
    }
    while (i < idx) {
        uint32_t next = fat32_next_cluster(vol, cl);
        if (next < 2 || next >= 0x0FFFFFF8) {
            next = fat32_alloc_cluster(vol);
            if (next < 2) {
                return -1;
            }
            fat32_set_cluster(vol, cl, next);
            fat32_set_cluster(vol, next, 0x0FFFFFFF);
            if (fat32_zero_cluster(vol, next) != 0) {
                return -1;
            }
        }
        cl = next;
        i++;
    }
    f->clus = cl;
    f->clus_idx = idx;
    return 0;
}

static int fat32_create_empty(fat32_vol_t *vol, uint32_t dir_cluster, const char *name,
                              fat32_file_t *out) {
    uint32_t slot_lba;
    int slot_off;
    uint8_t *e;

    if (fat32_find_dir_slot(vol, dir_cluster, &slot_lba, &slot_off) != 0) {
        return -1;
    }
    if (read_sector_vol(vol, slot_lba) != 0) {
        return -1;
    }
    e = sector_buf + slot_off;
    memset(e, 0, 32);
    name_to_83(name, e);
    e[11] = 0x20;
    if (write_sector_vol(vol, slot_lba) != 0) {
        return -1;
    }
    memset(out, 0, sizeof(*out));
    out->dir_cluster = dir_cluster;
    out->dir_lba = slot_lba;
    out->dir_off = slot_off;
    out->writable = 1;
    out->clus_idx = 0xFFFFFFFFu;
    return 0;
}

int fat32_file_open(fat32_vol_t *vol, uint32_t dir_cluster, const char *name,
                    int flags, fat32_file_t *out) {
    uint32_t cluster = 0;
    uint8_t attr = 0;
    uint32_t size = 0;
    uint32_t dir_lba = 0;
    int dir_off = 0;
    int found;

    if (!vol || !name || !name[0] || !out) {
        return -1;
    }
    if (!(flags & (F_READ | F_WRITE))) {
        return -1;
    }
    if ((flags & (F_CREATE | F_TRUNC | F_APPEND)) && !(flags & F_WRITE)) {
        return -1;
    }
    if (strcmp(name, ".") == 0 || strcmp(name, "..") == 0) {
        return -1;
    }

    memset(out, 0, sizeof(*out));
    found = lookup_dirent(vol, dir_cluster, name, &cluster, &attr, &size,
                          &dir_lba, &dir_off) == 0;
    if (found) {
        if (attr & FAT_ATTR_DIR) {
            return -1;
        }
        if (flags & F_TRUNC) {
            if (cluster >= 2) {
                fat32_free_chain(vol, cluster);
            }
            cluster = 0;
            size = 0;
            if (read_sector_vol(vol, dir_lba) != 0) {
                return -1;
            }
            wr16(sector_buf + dir_off + 26, 0);
            wr16(sector_buf + dir_off + 20, 0);
            wr32(sector_buf + dir_off + 28, 0);
            if (write_sector_vol(vol, dir_lba) != 0) {
                return -1;
            }
        }
        out->dir_cluster = dir_cluster;
        out->start_cluster = cluster;
        out->size = size;
        out->pos = (flags & F_APPEND) ? size : 0;
        out->clus = cluster;
        out->clus_idx = cluster >= 2 ? 0 : 0xFFFFFFFFu;
        out->dir_lba = dir_lba;
        out->dir_off = dir_off;
        out->writable = (flags & F_WRITE) != 0;
        out->append = (flags & F_APPEND) != 0;
        return 0;
    }

    if (!(flags & F_CREATE)) {
        return -1;
    }
    if (fat32_create_empty(vol, dir_cluster, name, out) != 0) {
        return -1;
    }
    out->append = (flags & F_APPEND) != 0;
    return 0;
}

int fat32_file_seek(fat32_file_t *f, uint32_t offset) {
    if (!f) {
        return -1;
    }
    f->pos = offset;
    return 0;
}

int fat32_file_read(fat32_vol_t *vol, fat32_file_t *f, void *buf, uint32_t cap,
                    uint32_t *out_len) {
    uint8_t *out = (uint8_t *)buf;
    uint32_t done = 0;
    uint32_t csize;

    if (out_len) {
        *out_len = 0;
    }
    if (!vol || !f || !buf) {
        return -1;
    }
    if (f->pos >= f->size || cap == 0) {
        return 0;
    }
    if (cap > f->size - f->pos) {
        cap = f->size - f->pos;
    }
    csize = fat32_cluster_size(vol);
    while (done < cap) {
        uint32_t idx = f->pos / csize;
        uint32_t off = f->pos % csize;
        uint32_t n = cap - done;
        uint32_t room = csize - off;

        if (n > room) {
            n = room;
        }
        if (fat32_cluster_at(vol, f, idx) != 0) {
            return -1;
        }
        if (fat32_rw_cluster(vol, f->clus, off, out + done, 0, n) != 0) {
            return -1;
        }
        f->pos += n;
        done += n;
    }
    if (out_len) {
        *out_len = done;
    }
    return 0;
}

static int fat32_write_bytes(fat32_vol_t *vol, fat32_file_t *f, const uint8_t *data,
                             uint32_t len, uint32_t *out_len) {
    uint32_t done = 0;
    uint32_t csize = fat32_cluster_size(vol);

    while (done < len) {
        uint32_t idx = f->pos / csize;
        uint32_t off = f->pos % csize;
        uint32_t n = len - done;
        uint32_t room = csize - off;

        if (n > room) {
            n = room;
        }
        if (fat32_ensure_cluster(vol, f, idx) != 0) {
            if (out_len) {
                *out_len = done;
            }
            return done ? 0 : -1;
        }
        if (fat32_rw_cluster(vol, f->clus, off, 0, data + done, n) != 0) {
            return -1;
        }
        f->pos += n;
        done += n;
        if (f->pos > f->size) {
            f->size = f->pos;
            f->dirty = 1;
        }
    }
    if (out_len) {
        *out_len = done;
    }
    return 0;
}

int fat32_file_write(fat32_vol_t *vol, fat32_file_t *f, const void *data, uint32_t len,
                     uint32_t *out_len) {
    if (out_len) {
        *out_len = 0;
    }
    if (!vol || !f || !f->writable || (len > 0 && !data)) {
        return -1;
    }
    if (f->append) {
        f->pos = f->size;
    }
    if (f->pos > f->size) {
        uint32_t target = f->pos;
        uint8_t z[512];

        memset(z, 0, sizeof(z));
        f->pos = f->size;
        while (f->pos < target) {
            uint32_t n = target - f->pos;
            uint32_t got = 0;
            if (n > sizeof(z)) {
                n = (uint32_t)sizeof(z);
            }
            if (fat32_write_bytes(vol, f, z, n, &got) != 0 || got == 0) {
                return -1;
            }
        }
    }
    if (len == 0) {
        return 0;
    }
    return fat32_write_bytes(vol, f, (const uint8_t *)data, len, out_len);
}

int fat32_file_close(fat32_vol_t *vol, fat32_file_t *f) {
    int rc = 0;

    if (!f) {
        return -1;
    }
    if (f->dirty && f->writable && vol) {
        rc = fat32_sync_dirent(vol, f);
    }
    memset(f, 0, sizeof(*f));
    return rc;
}

static int fat32_write_dir_sector(fat32_vol_t *vol, uint32_t cluster,
                                  uint32_t parent_cluster) {
    uint32_t lba = cluster_lba(vol, cluster);
    for (uint32_t s = 0; s < vol->sectors_per_cluster; s++) {
        memset(sector_buf, 0, vol->bytes_per_sector);
        if (s == 0) {
            uint8_t *dot = sector_buf;
            dot[0] = '.';
            for (int i = 1; i < 11; i++) {
                dot[i] = ' ';
            }
            dot[11] = FAT_ATTR_DIR;
            wr16(dot + 26, cluster & 0xFFFF);
            wr16(dot + 20, (uint16_t)(cluster >> 16));

            uint8_t *dotdot = sector_buf + 32;
            dotdot[0] = '.';
            dotdot[1] = '.';
            for (int i = 2; i < 11; i++) {
                dotdot[i] = ' ';
            }
            dotdot[11] = FAT_ATTR_DIR;
            wr16(dotdot + 26, parent_cluster & 0xFFFF);
            wr16(dotdot + 20, (uint16_t)(parent_cluster >> 16));
        }
        if (write_sector_vol(vol, lba + s) != 0) {
            return -1;
        }
    }
    fat32_set_cluster(vol, cluster, 0x0FFFFFF8);
    return 0;
}

int fat32_mkdir(fat32_vol_t *vol, uint32_t parent_cluster, const char *name) {
    uint32_t existing = 0;
    uint8_t attr = 0;
    uint32_t size = 0;

    if (*name == 0 || strcmp(name, ".") == 0 || strcmp(name, "..") == 0) {
        return -1;
    }
    if (fat32_lookup(vol, parent_cluster, name, &existing, &attr, &size) == 0) {
        return -1;
    }

    uint32_t cluster = fat32_alloc_cluster(vol);
    if (cluster < 2) {
        return -1;
    }

    if (fat32_write_dir_sector(vol, cluster, parent_cluster) != 0) {
        fat32_set_cluster(vol, cluster, 0);
        return -1;
    }

    uint32_t slot_lba;
    int slot_off;
    if (fat32_find_dir_slot(vol, parent_cluster, &slot_lba, &slot_off) != 0) {
        fat32_free_chain(vol, cluster);
        return -1;
    }
    if (read_sector_vol(vol, slot_lba) != 0) {
        fat32_free_chain(vol, cluster);
        return -1;
    }

    uint8_t *e = sector_buf + slot_off;
    name_to_83(name, e);
    e[11] = FAT_ATTR_DIR;
    wr16(e + 26, cluster & 0xFFFF);
    wr16(e + 20, (uint16_t)(cluster >> 16));
    wr32(e + 28, 0);
    if (write_sector_vol(vol, slot_lba) != 0) {
        fat32_free_chain(vol, cluster);
        return -1;
    }
    return 0;
}

typedef struct {
    int empty;
} dir_empty_ctx_t;

static int dir_empty_cb(const char *name, uint8_t attr, uint32_t size, void *ctx) {
    (void)attr;
    (void)size;
    dir_empty_ctx_t *c = (dir_empty_ctx_t *)ctx;
    if (name[0] == '.' &&
        (name[1] == 0 || (name[1] == '.' && name[2] == 0))) {
        return 0;
    }
    c->empty = 0;
    return 1;
}

static int fat32_dir_is_empty(fat32_vol_t *vol, uint32_t cluster) {
    dir_empty_ctx_t ctx = {1};
    fat32_list_dir(vol, cluster, dir_empty_cb, &ctx);
    return ctx.empty;
}

int fat32_delete(fat32_vol_t *vol, uint32_t dir_cluster, const char *name) {
    if (*name == 0 || strcmp(name, ".") == 0 || strcmp(name, "..") == 0) {
        return -1;
    }

    uint32_t scan = dir_cluster;
    while (scan >= 2 && scan < 0x0FFFFFF8) {
        uint32_t lba = cluster_lba(vol, scan);
        for (uint32_t s = 0; s < vol->sectors_per_cluster; s++) {
            if (read_sector_vol(vol, lba + s) != 0) {
                return -1;
            }
            for (int i = 0; i < 512; i += 32) {
                uint8_t *e = sector_buf + i;
                if (e[0] == 0) {
                    return -1;
                }
                if (e[0] == 0xE5 || e[11] == 0x0F) {
                    continue;
                }
                char nm[13];
                format_83_name(e, nm, sizeof(nm));
                if (!match_component(nm, name)) {
                    continue;
                }

                uint8_t attr = e[11];
                uint32_t cl = ((uint32_t)rd16(e + 20) << 16) | rd16(e + 26);
                uint32_t ent_lba = lba + s;
                int ent_off = i;

                if (attr & FAT_ATTR_DIR) {
                    if (!fat32_dir_is_empty(vol, cl)) {
                        return -2;
                    }
                    fat32_free_chain(vol, cl);
                } else if (cl >= 2) {
                    fat32_free_chain(vol, cl);
                }

                /* free_chain / dir_is_empty reuse sector_buf (including the
                 * FSInfo sector, which starts with "RRaA"). Reload before
                 * marking the slot deleted. */
                if (read_sector_vol(vol, ent_lba) != 0) {
                    return -1;
                }
                sector_buf[ent_off] = 0xE5;
                return write_sector_vol(vol, ent_lba);
            }
        }
        scan = fat32_next_cluster(vol, scan);
    }
    return -1;
}
